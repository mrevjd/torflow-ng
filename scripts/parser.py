...
def dl_server_descriptors(year, month):
    ...
    try:
        ...
        if request.code != 200:
            print("  [-] Unable to fetch server descriptors %s at %s" % (filename, url))
            return None
    except Exception as e:
...
def create_csv_file(year, month, day):
    ...
    csv_filename = "data/relays-%s-%s-%s-00-00-00.csv" % (year, month, day)
...
def write_csv_data(
    consensus, sd_path, prev_sd_path, ei_path, prev_ei_path, year, month, day
):
    ...
    for desc in consensus.routers.values():
        ...
        csv_fp.write(
            ...
            % (
                ...
                desc.address,
                desc.or_port,
                float(sd.observed_bandwidth / 1000.0 / 1000.0),
                entry_ips,
                dir_ips,
                sd.uptime,
                lon,
...
def make_monthly_csv(year, month, day):
    ...
    for dir_day in os.listdir(f"./{consensus_path[:-7]}"):
        print(f"Processing day: {dir_day}")
        try:
            str_day = f"0{day}" if day < 10 else str(day)
            if day != 0 and str_day != dir_day:
                print(f"Skipping day {dir_day} (doesn't match {str_day})")
                continue

            match_found = True
            consensus_pathname = f"./consensuses/consensuses-{year}-{str_month}/{dir_day}/{year}-{str_month}-{dir_day}-00-00-00-consensus"

            print(f"  [+] Reading consensus {consensus_pathname}")

            if not os.path.exists(consensus_pathname):
                print(f"  [-] Consensus {consensus_pathname} not found. Skipping!")
                continue

            try:
                consensus = next(
                    parse_file(
                        consensus_pathname, document_handler=DocumentHandler.DOCUMENT
                    )
                )
            except Exception as e:
                print(
                    f"  [-] Error reading consensus {consensus_pathname}: {e}. Skipping!"
                )
                continue
            ...
            try:
                ...
                print(f"  [-] Error in write_csv_data for {dir_day}: {e}")
                continue

        except Exception as e:
            print(f"  [-] Unexpected error processing {dir_day}: {e}")
    ...
    print("  [+] Finished processing all available consensus files.")
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(GEOIP_FILENAME):
        print(
            "%s not found. It must be in the same directory as this script."
            % GEOIP_FILENAME
        )
